...
inline void TriangularMatrix::AddPoint(PointF^ newPoint, int index) {
	...
	for (int j = i-1; j>=0; j--) {
		double value = matrix[i][j+1] - matrix[i-1][j];
...
bool TriangularMatrix::Expand(PointF ^newPoint) {
	...
	if (newPoint->X < points[0]->X) {
		...
		///*option 2*/ {
		//array<PointF^>::Resize(points, points->Length+1);
		...
		//add column to the left of the matrix
		...
		for (int i = 1; i<matrix->Length; ++i) {
			double value = matrix[i][1] - matrix[i-1][0];
